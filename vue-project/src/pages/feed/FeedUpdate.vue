...
onMounted(async () => {
  try {
    ...
    curTitle.value = data.ok.title ?? ''
    curDescription.value = data.ok.description ?? ''
    title.value = curTitle.value
    description.value = curDescription.value
  } catch (err) {
    console.error(err)
    ...
    <template v-else>
      ...
      <div class="card shadow-sm mb-4">
        ...
        <div class="card-body">
          ...
          <div class="preview-content" v-html="curDescription || '<em>내용 없음</em>'"></div>
        </div>
      </div>
